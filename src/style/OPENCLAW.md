...
├── settings/       # base (shared .opencodian-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: All classes use `.opencodian-` prefix
- **BEM-lite**: `.opencodian-{block}`, `.opencodian-{block}-{element}`, `.opencodian-{block}--{modifier}`
- **No `!important`**: Avoid unless overriding Obsidian defaults
- **CSS variables**: Use Obsidian's `--background-*`, `--text-*`, `--interactive-*` tokens

## Brand Colors

- **Primary Brand**: `#4A90E2` (Blue - represents "Open")
- **Error**: `#dc3545` (Red)
- **Compact Mode**: `#5bc0de` (Cyan)
