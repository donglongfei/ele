...
function addHotkeySettingRow(
  ...
  const item = containerEl.createDiv({ cls: 'opencodian-hotkey-item' });
  item.createSpan({ cls: 'opencodian-hotkey-name', text: t(`${translationPrefix}.name` as TranslationKey) });
  if (hotkey) {
    item.createSpan({ cls: 'opencodian-hotkey-badge', text: hotkey });
  }
...
export class OpenCodianSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl).setName(t('settings.gateway')).setHeading();

    new Setting(containerEl)
      .setName(t('settings.gatewayUrl.name'))
      .setDesc(t('settings.gatewayUrl.desc'))
      .addText((text) => {
        text
          .setPlaceholder('ws://127.0.0.1:18789')
          .setValue(this.plugin.settings.openClawGatewayUrl || 'ws://127.0.0.1:18789')
          .onChange(async (value) => {
            this.plugin.settings.openClawGatewayUrl = value.trim();
            await this.plugin.saveSettings();
            // Notify user that they should restart conversations
            new Notice('Gateway URL updated. Please restart active conversations.');
          });
    ...
    new Setting(containerEl)
      ...
      .setDesc('Authentication token for OpenClaw Gateway (required for connection)')
      .addText((text) => {
        text
          .setPlaceholder('671b4dba2ec61e3dd90151e43b3d073f843cb666e7194ba1')
          .setValue(this.plugin.settings.openClawAuthToken || '')
          .onChange(async (value) => {
            this.plugin.settings.openClawAuthToken = value.trim();
            await this.plugin.saveSettings();
            new Notice('Auth token updated. Please restart active conversations.');
          });
        text.inputEl.style.width = '100%';
        text.inputEl.type = 'password';  // Hide token
      });

    new Setting(containerEl)
      .setName('Gateway Session ID')
      .setDesc('Session ID to use for agent requests. Leave empty to auto-detect from ~/.openclaw/agents/main/sessions/sessions.json (recommended). Session resets daily at 4:00 AM by default.')
      .addText((text) => {
        text
          .setPlaceholder('Leave empty for auto-detect (recommended)')
          .setValue(this.plugin.settings.openClawSessionId || '')
          .onChange(async (value) => {
            this.plugin.settings.openClawSessionId = value.trim();
            await this.plugin.saveSettings();
            new Notice('Session ID updated. Please restart active conversations.');
          });
        text.inputEl.style.width = '100%';
      });
    ...
    if (deviceToken) {
      ...
      pairingStatus.style.color = 'var(--text-success)';
      pairingStatus.innerHTML = `✅ <span>Device paired (token: ${deviceToken.substring(0, 16)}...)</span>`;
    }

    // Gateway health check indicator
    const gatewayStatusEl = containerEl.createDiv({ cls: 'opencodian-gateway-status' });
    this.checkGatewayHealth(gatewayStatusEl);
    ...
    new Setting(containerEl)
      ...
      .addText((text) => {
        ...
        text.inputEl.addClass('opencodian-settings-media-input');
        text.inputEl.addEventListener('blur', () => this.restartServiceForPromptChange());
      });
    ...
    const hotkeyGrid = containerEl.createDiv({ cls: 'opencodian-hotkey-grid' });
    ...
    const agentsDesc = containerEl.createDiv({ cls: 'opencodian-sp-settings-desc' });
    agentsDesc.createEl('p', {
      text: t('settings.subagents.desc'),
      cls: 'setting-item-description',
    });

    const agentsContainer = containerEl.createDiv({ cls: 'opencodian-agents-container' });
    new AgentSettings(agentsContainer, this.plugin);

    new Setting(containerEl).setName(t('settings.mcpServers.name')).setHeading();

    const mcpDesc = containerEl.createDiv({ cls: 'opencodian-mcp-settings-desc' });
    mcpDesc.createEl('p', {
    ...
    this.contextLimitsContainer = containerEl.createDiv({ cls: 'opencodian-context-limits-container' });
    this.renderContextLimitsSection();

    const envSnippetsContainer = containerEl.createDiv({ cls: 'opencodian-env-snippets-container' });
    new EnvSnippetManager(envSnippetsContainer, this.plugin, () => {
      this.renderContextLimitsSection();
    });

    new Setting(containerEl).setName('Thinking & Reasoning').setHeading();

    // Reasoning Toggle - Show thinking process
    new Setting(containerEl)
      .setName('Show Reasoning')
      .setDesc('Display the model\'s thinking process in responses. This is independent from thinking level.')
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.reasoningEnabled ?? false)
          .onChange(async (value) => {
            this.plugin.settings.reasoningEnabled = value;
            await this.plugin.saveSettings();

            // Sync to OpenClaw Gateway via active tab's service
            try {
              ...
              if (activeTab?.service) {
                const session = activeTab.service.sessionManager.getActiveSession();
                if (session) {
                  await activeTab.service.setReasoning(value, session.channelKey);
                } else {
                  console.warn('[Settings] No active session for setReasoning');
                }
              } else {
                console.warn('[Settings] No active tab service available for setReasoning');
              }
            } catch (error) {
              console.error('[Settings] Failed to sync reasoning to OpenClaw:', error);
            }
          })
      );
    ...
    const bangBashValidationEl = containerEl.createDiv({ cls: 'opencodian-bang-bash-validation' });
    ...
    const maxTabsWarningEl = containerEl.createDiv({ cls: 'opencodian-max-tabs-warning' });
  ...
  private renderContextLimitsSection(): void {
    ...
    const headerEl = container.createDiv({ cls: 'opencodian-context-limits-header' });
    headerEl.createSpan({ text: t('settings.customContextLimits.name'), cls: 'opencodian-context-limits-label' });

    const descEl = container.createDiv({ cls: 'opencodian-context-limits-desc' });
    descEl.setText(t('settings.customContextLimits.desc'));

    const listEl = container.createDiv({ cls: 'opencodian-context-limits-list' });

    for (const modelId of uniqueModelIds) {
      const currentValue = this.plugin.settings.customContextLimits?.[modelId];

      const itemEl = listEl.createDiv({ cls: 'opencodian-context-limits-item' });

      const nameEl = itemEl.createDiv({ cls: 'opencodian-context-limits-model' });
      nameEl.setText(modelId);

      const inputWrapper = itemEl.createDiv({ cls: 'opencodian-context-limits-input-wrapper' });

      const inputEl = inputWrapper.createEl('input', {
        type: 'text',
        placeholder: '200k',
        cls: 'opencodian-context-limits-input',
        value: currentValue ? formatContextLimit(currentValue) : '',
      });

      // Validation element
      const validationEl = inputWrapper.createDiv({ cls: 'opencodian-context-limit-validation' });
      ...
      inputEl.addEventListener('input', async () => {
        ...
        if (!trimmed) {
          ...
          inputEl.classList.remove('opencodian-input-error');
        } else {
          const parsed = parseContextLimit(trimmed);
          if (parsed === null) {
            validationEl.setText(t('settings.customContextLimits.invalid'));
            validationEl.style.display = 'block';
            inputEl.classList.add('opencodian-input-error');
  ...
  /**
   * Check OpenClaw Gateway health and display status indicator
   */
  private async checkGatewayHealth(containerEl: HTMLElement): Promise<void> {
    containerEl.empty();
    containerEl.style.marginTop = '-0.5em';
    ...
    try {
      // Attempt to connect to Gateway (with timeout)
      const gatewayUrl = this.plugin.settings.openClawGatewayUrl || 'ws://127.0.0.1:18789';
      ...
      const checkPromise = new Promise<boolean>((resolve) => {
        try {
          ...
          ws.onmessage = () => {
            // Received any message = connection successful
            if (!resolved) {
              resolved = true;
              ws.close();
              resolve(true);
      ...
      if (isHealthy) {
        statusEl.innerHTML = '✅ <span style="color: var(--text-success);">Gateway is running</span>';
      } else {
 ...
 */
export const ClaudianSettingTab = OpenCodianSettingTab;
