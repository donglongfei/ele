...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.bashOutputContainerEl.className = 'ele-status-panel-bash';
    this.bashOutputContainerEl.style.display = 'none';

    this.bashHeaderEl = document.createElement('div');
    this.bashHeaderEl.className = 'ele-tool-header claudian-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'ele-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'ele-status-panel-todos';
    ...
    this.todoHeaderEl.className = 'ele-status-panel-header';
    ...
    this.todoContentEl.className = 'ele-status-panel-content claudian-todo-list-container';
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'ele-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'ele-status-panel-current';
  ...
  private renderSubagentStatus(): void {
    ...
    for (let i = 0; i < completedSubagents.length; i++) {
      ...
      rowEl.className = showRunningOnThisRow
        ? 'ele-status-panel-done-row claudian-status-panel-combined-row'
        : 'ele-status-panel-done-row';

      // Bot icon
      const botIconEl = document.createElement('span');
      botIconEl.className = 'ele-status-panel-icon claudian-status-panel-bot-icon';
      setIcon(botIconEl, getToolIcon(TOOL_TASK));
      rowEl.appendChild(botIconEl);

      // Description text
      const textEl = document.createElement('span');
      textEl.className = 'ele-status-panel-done-text';
      textEl.textContent = this.truncateDescription(subagent.description);
      rowEl.appendChild(textEl);

      // Green tick icon
      const iconEl = document.createElement('span');
      iconEl.className = 'ele-status-panel-icon claudian-status-panel-done-icon';
      ...
      if (showRunningOnThisRow) {
        ...
        runningEl.className = 'ele-status-panel-running-text';
    ...
    if (runningSubagents.length > 0 && completedSubagents.length === 0) {
      const rowEl = document.createElement('div');
      rowEl.className = 'ele-status-panel-running-row';

      // Count text
      const textEl = document.createElement('span');
      textEl.className = 'ele-status-panel-running-text';
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    headerIconEl.className = 'ele-tool-icon';
    ...
    headerLabelEl.className = 'ele-tool-label';
    ...
    previewEl.className = 'ele-tool-current';
    ...
    summaryStatusEl.className = 'ele-tool-status';
    ...
    actionsEl.className = 'ele-status-panel-bash-actions';
  ...
  private renderBashEntry(info: PanelBashOutput): HTMLElement {
    ...
    entryLabelEl.className = 'ele-tool-label';
    ...
    entryStatusEl.className = 'ele-tool-status';
    ...
    contentEl.className = 'ele-tool-content';
    ...
    const rowEl = document.createElement('div');
    rowEl.className = 'ele-tool-result-row';

    const textEl = document.createElement('span');
    textEl.className = 'ele-tool-result-text';
  ...
  private appendActionButton(
    ...
    el.className = `ele-status-panel-bash-action claudian-status-panel-bash-action-${name}`;
