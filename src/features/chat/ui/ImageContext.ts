...
export class ImageContextManager {
  ...
  constructor(
    ...
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'ele-image-preview' });
  ...
  private setupDragAndDrop() {
    ...
    this.dropOverlay = inputWrapper.createDiv({ cls: 'ele-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'ele-drop-content' });
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'ele-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'ele-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'ele-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'ele-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'ele-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'ele-image-remove' });
    removeEl.setText('\u00D7');
