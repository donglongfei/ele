...
export function renderTodoItems(
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `ele-todo-item claudian-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'ele-todo-status-icon' });
    icon.setAttribute('aria-hidden', 'true');
    setIcon(icon, getTodoStatusIcon(todo.status));

    const text = item.createSpan({ cls: 'ele-todo-text' });
