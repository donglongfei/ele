...
function renderWebSearchExpanded(container: HTMLElement, result: string): void {
  ...
  const linksEl = container.createDiv({ cls: 'ele-tool-lines' });
  for (const link of parsed.links) {
    const linkEl = linksEl.createEl('a', { cls: 'ele-tool-link' });
    linkEl.setAttribute('href', link.url);
    linkEl.setAttribute('target', '_blank');
    linkEl.setAttribute('rel', 'noopener noreferrer');

    const iconEl = linkEl.createSpan({ cls: 'ele-tool-link-icon' });
    setIcon(iconEl, 'external-link');

    linkEl.createSpan({ cls: 'ele-tool-link-title', text: link.title });
  }

  if (parsed.summary) {
    const summaryEl = container.createDiv({ cls: 'ele-tool-web-summary' });
...
function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  ...
  if (lines.length === 0) {
    container.createDiv({ cls: 'ele-tool-empty', text: 'No matches found' });
    return;
  }
...
function renderLinesExpanded(
  ...
  const linesEl = container.createDiv({ cls: 'ele-tool-lines' });
  for (const line of displayLines) {
    const stripped = line.replace(/^\s*\d+→/, '');
    const lineEl = linesEl.createDiv({ cls: 'ele-tool-line' });
...
function renderToolSearchExpanded(container: HTMLElement, result: string): void {
  ...
  for (const name of toolNames) {
    const lineEl = container.createDiv({ cls: 'ele-tool-search-item' });
    const iconEl = lineEl.createSpan({ cls: 'ele-tool-search-icon' });
...
export function renderExpandedContent(container: HTMLElement, toolName: string, result: string | undefined): void {
  if (!result) {
    container.createDiv({ cls: 'ele-tool-empty', text: 'No result' });
    return;
  }
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'ele-tool-status';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('ele-todo-panel-content');
  container.addClass('ele-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'ele-tool-result-item' });
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'ele-tool-call' });

  const header = toolEl.createDiv({ cls: 'ele-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');

  const iconEl = header.createSpan({ cls: 'ele-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = header.createSpan({ cls: 'ele-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'ele-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'ele-tool-status' });

  const content = toolEl.createDiv({ cls: 'ele-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    bodyEl.createDiv({
      ...
      cls: answer ? 'ele-ask-review-a-text' : 'ele-ask-review-empty',
    });
  }
...
function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'ele-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'ele-tool-result-text' });
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'ele-tool-current' });
