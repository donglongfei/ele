...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('ele-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'ele-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'ele-fork-target-option', text: label });
